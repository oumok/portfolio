...
function openProject(project){
  ...
  // init lightbox with concept images if exists
...
function createGalleryGroup(items,type,active,title){
  ...
  items.forEach(i=>{
    ...
    img.addEventListener('click',()=>{ lbImages=Array.from(div.querySelectorAll('img')).map(x=>x.src); lbIndex=lbImages.indexOf(src); openLightbox(src,caption); });
    div.appendChild(img);
  });
  return div;
}
...
document.addEventListener('keydown',e=>{
  ...
  } else if(modal()?.classList.contains('is-open')){ if(e.key==='Escape') closeModal(); }
});
...
(async function loadProjects(){
  ...
  if(dataEl && dataEl.textContent.trim()){ try{ data=JSON.parse(dataEl.textContent);}catch(e){console.error(e);} }
  if(!data){ const res=await fetch('./projects.json'); data=await res.json(); }
